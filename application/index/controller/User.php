        ...
        if ($this->request->isPost()) {
            ...
            if ($card['valid_time'] > 0) {
                if ($_G['member']['out_time'] >= $this->request->time()) {
                    $update['out_time'] = $_G['member']['out_time'] + $card['valid_time'] * 3600;
                } else if ($_G['member']['out_time'] > 0) {
                    $update['out_time'] = $this->request->time() + $card['valid_time'] * 3600;
                }
            } else if ($card['valid_time'] == -1) {
                $update['out_time'] = 0;
            }
            if ($card['account_times'] > 0) {
                $update['parse_max_times'] = $_G['member']['parse_max_times'] == 0 ? 0 : $card['account_times'] + $_G['member']['parse_max_times'];
            } else if ($card['account_times'] == -1) {
                $update['parse_max_times'] = 0;
            }
            foreach ((array) $card['access_times'] as $site_id => $access) {
                if (!empty($access->day)) {
                    $update['site_access']->$site_id->day += $access->day;
                }
                if (!empty($access->all)) {
                    $update['site_access']->$site_id->all += $access->all;
                }
            }
